...
struct SLongitudeLatitude : public SVector<T, 2, SLongitudeLatitude<T> >
{
	...
	public:
		...
		virtual SLongitudeLatitude OffsetFromStart(SLongitudeLatitude const &, SVector2<T> const &) { return SLongitudeLatitude(); }
	...
	public:
		...
		SLongitudeLatitude OffsetFromStart(SLongitudeLatitude const & start, SVector2<T> const & offset) {
			T EarthRadius = (T) 6378.137 * 1000.0; // meters
			T Bearing = ArcTan(offset.Y, offset.X);
			Bearing = -Bearing + Constants<T>::Pi() / 2;
			if (Bearing < 0) { Bearing += 2 * Constants<T>::Pi(); }
			T AngularDistance = offset.Length() / EarthRadius;

			T Lat1 = DegreesToRadians(start.Latitude);
			T Lon1 = DegreesToRadians(start.Longitude);

			T Lat2 = asin(Sin(Lat1) * Cos(AngularDistance) + 
				Cos(Lat1) * Sin(AngularDistance) * cos(Bearing));
			T Lon2 = Lon1 +
				ArcTan(Sin(Bearing) * Sin(AngularDistance) * cos(Lat1),
				Cos(AngularDistance) - Sin(Lat1) * sin(Lat2));

			return SLongitudeLatitude(RadiansToDegrees(Lon2), RadiansToDegrees(Lat2));
		}
